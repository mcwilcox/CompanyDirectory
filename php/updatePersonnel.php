<?php
	...
	$stmt = $conn->prepare("SELECT * FROM personnel WHERE firstName = ? AND lastName = ? AND jobTitle = ? AND email = ? AND departmentID = ?");
	...
	while ($row = mysqli_fetch_assoc($checkExistingEntry)) {
	...
	if(count($existingEntry) == 0) {
		...
		$stmt->bind_param("ssssss", $firstName, $lastName, $email, $jobTitle, $departmentID, $id);
		$stmt->execute();
